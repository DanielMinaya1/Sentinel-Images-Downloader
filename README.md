...
It returns `None` if nothing qualifies. Raw Sentinel-2 data goes to a temporary directory that's deleted afterwards, leaving only the JPG in `output_dir` (pass `download_dir=` to keep it). For a point or line, pass `buffer_meters=` since it has no area of its own.
